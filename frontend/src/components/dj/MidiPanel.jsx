...
import { X, Gamepad2, Plug, PlugZap, Activity, Check } from "lucide-react";
...
export default function MidiPanel({ open, onClose }) {
  ...
  const [monitor, setMonitor] = useState([]);         // last ~20 incoming messages
  const [lastMatched, setLastMatched] = useState(null);
  const [filter, setFilter] = useState("");
  const monitorRef = useRef([]);

  // Request MIDI on open
  useEffect(() => {
    if (!open) return;
    let cancelled = false;
    (async () => {
      ...
      setDevices(list);
      // Auto-reconnect persisted device
  ...
  // Live MIDI monitor + matched-mapping indicator (panel always listens while open)
  useEffect(() => {
    if (!open) return;
    const h = (e) => {
      const msg = { ...e.detail };
      msg.sig = midiSignature(msg);
      monitorRef.current = [msg, ...monitorRef.current].slice(0, 20);
      setMonitor(monitorRef.current);
      // Check if this matches a mapping
  ...
  // Learn: first message captures the signature
  useEffect(() => {
  ...
  if (!open) return null;
  const filtered = MIDI_CONTROLS.filter((c) =>
  ...
  return (
    <div className="fixed inset-0 z-[80] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" data-testid="midi-panel">
      <div className="bg-[#141414] border border-white/10 rounded-lg w-full max-w-4xl p-6 relative shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="mb-4 flex flex-col gap-2">
          ...
          {devices.length === 0 && !error && (
            <span className="text-xs text-[#52525B]">
              Plug in a DJ controller (USB) — DJ Lab requests MIDI permission only.
            </span>
          )}
        </div>

        {/* Live monitor */}
        <div className="mb-4 border border-white/10 rounded p-3 bg-black/40" data-testid="midi-monitor">
          <div className="flex items-center justify-between mb-2">
            <span className="label-tiny flex items-center gap-1.5">
              <Activity className="w-3 h-3" />
              Live MIDI Monitor
            </span>
            <div className="flex items-center gap-2 text-[10px] font-mono-dj">
              {matchedIsRecent ? (
                <span className="text-[#FF1F1F] flex items-center gap-1 animate-pulse">
                  <Check className="w-3 h-3" /> {lastMatched.controlId} ← {lastMatched.sig} / {lastMatched.data2}
                </span>
              ) : (
                <span className="text-[#52525B]">Waiting for messages…</span>
              )}
            </div>
          </div>
          <div className="grid grid-cols-6 gap-1 text-[10px] font-mono-dj text-[#A1A1AA] max-h-24 overflow-y-auto">
            <div className="text-[#52525B]">TYPE</div>
            <div className="text-[#52525B]">CH</div>
            <div className="text-[#52525B]">D1</div>
            <div className="text-[#52525B]">D2</div>
            <div className="text-[#52525B]">SIG</div>
            <div className="text-[#52525B]">MATCH</div>
            {monitor.map((m, i) => {
              const match = Object.entries(midi.mappings).find(([, mm]) => mm.signature === m.sig);
              return (
                <div key={i} className="contents">
                  <div>{typeLabel(m.type)}</div>
                  <div>{m.channel + 1}</div>
                  <div>{m.data1}</div>
                  <div>{m.data2}</div>
                  <div className="text-[#FF1F1F]">{m.sig}</div>
                  <div className="truncate" title={match?.[0] || ""}>{match?.[0] || "—"}</div>
                </div>
              );
            })}
          </div>
          {monitor.length === 0 && (
            <div className="text-[10px] text-[#52525B] italic">
              Move a knob or press a button on the controller — messages should appear here.
            </div>
          )}
        </div>
        ...
        <div className="mb-2 flex items-center gap-2">
          ...
          {Object.keys(midi.mappings).length > 0 && (
            <button
              onClick={() => Object.keys(midi.mappings).forEach((k) => clearMidiMapping(k))}
              className="px-2 py-1.5 rounded border border-white/15 text-[10px] uppercase tracking-wider text-[#A1A1AA] hover:text-white"
              data-testid="midi-clear-all"
            >
              Clear all
            </button>
          )}
        </div>
        <div className="flex-1 overflow-y-auto border-t border-white/10 pt-2">
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-[#141414]">
              <tr className="label-tiny text-left">
                <th className="px-2 py-1">Control</th>
                <th className="px-2 py-1">Mapping</th>
                <th className="px-2 py-1 w-36"></th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((c) => {
