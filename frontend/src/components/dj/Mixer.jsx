...
import { Mic, Square, Download, FolderOpen, Gamepad2 } from "lucide-react";
...
export default function Mixer({ deckChains, onOpenSaveSet, onOpenSavedSets, onOpenMidi }) {
  ...
  const start = async () => {
    ...
    startMasterRecording((blob, mime, durationSec) => {
      ...
      toast.success("Mix downloaded", {
        description: `${(blob.size / 1024 / 1024).toFixed(2)} MB — click to save set`,
    ...
    toast.message("Recording started", { description: "Capturing everything on the master bus." });
  };
  ...
  return (
    <div data-testid="mixer"
      className="flex flex-col gap-3 bg-[#0a0a0a] border-x border-white/5 p-4 items-center justify-between min-w-[200px] overflow-y-auto">
      {/* Master VU */}
      <div className="w-full text-center">
        <div className="label-tiny mb-2">MASTER</div>
        <div className="flex justify-center gap-1">
          {[0, 1].map((ch) => (
            <div key={ch} className="w-3 h-20 bg-[#1a1a1a] rounded overflow-hidden flex flex-col-reverse">
      ...
      <div className="flex flex-col items-center gap-1">
        ...
        <input type="range" min={0} max={1.2} step={0.01} value={masterVolume}
          ...
          className="fader-vert" style={{ height: 80 }} data-testid="master-volume" />
      </div>

      {/* Record + Save */}
      <div className="w-full flex flex-col items-center gap-2">
        <button data-testid="record-toggle" onClick={recording ? stop : start}
          className={`w-full px-4 py-2.5 rounded border-2 text-[11px] font-bold uppercase tracking-[0.2em] flex items-center justify-center gap-2 transition-all ${
            recording
        ...
        <div className="w-full flex gap-1">
          <button data-testid="save-set-open"
            ...
            className="flex-1 px-2 py-1.5 rounded border border-white/15 text-[10px] font-bold uppercase tracking-[0.15em] text-[#A1A1AA] hover:text-white hover:border-white/30">
            Save Set
          </button>
          <button data-testid="saved-sets-open"
            onClick={() => onOpenSavedSets?.()}
            className="px-2 py-1.5 rounded border border-white/15 text-[10px] font-bold uppercase tracking-[0.15em] text-[#A1A1AA] hover:text-white hover:border-white/30">
            <FolderOpen className="w-3 h-3" />
          </button>
          <button data-testid="midi-open" onClick={() => onOpenMidi?.()}
            className={`px-2 py-1.5 rounded border text-[10px] font-bold uppercase tracking-[0.15em] transition ${
              midi.enabled
                ? "border-[#D10A0A] text-[#FF1F1F] bg-[#D10A0A]/10"
