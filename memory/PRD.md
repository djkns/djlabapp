...
- **2026-02-24 — BPM cache in MongoDB.** Added `track_meta` collection + 2 endpoints (`GET /api/tracks/meta?key=`, `POST /api/tracks/meta`). `Deck.jsx` checks the cache before running analysis — instant cache hit on repeat loads (verified ~50% faster: 4.4s → 2.0s for 4MB tracks). `/api/tracks` listing now inlines cached BPMs in a single Mongo query so the library table shows BPM without re-analyzing on page refresh. Schema is future-proofed for `musical_key`, hot cues, etc.
- **2026-02-24 — Auto BPM detection (P1).** Added `web-audio-beat-detector@8.2.36` (pure-JS, no service). When a track loads on a deck, `Deck.jsx` fetches the audio bytes once, decodes them via the shared `AudioContext`, and runs `analyzeBPM(audioBuffer)` in a fire-and-forget background task. If the result lands within 60–200 BPM and the user hasn't swapped tracks, `baseBPM` is updated automatically (rounded to 1 decimal). A small orange `Activity` icon next to the BPM display indicates analysis in progress.
- **2026-02-24 — Reverted stacked beat-grid (user preference).** Restored per-deck `WaveSurfer` + the `<div ref={waveRef}>` block in `Deck.jsx`.
- **2026-02-24 — Vertical fader look/feel matched to crossfader.** Volume & tempo faders restyled to share the crossfader's chunky dark thumb, flat track, red glow.
