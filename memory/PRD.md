...
- **2026-02-24 — Reverted stacked beat-grid (user preference).** Removed `StackedWaveform.jsx` and the `<StackedWaveform />` mount. Restored per-deck `WaveSurfer` + the `<div ref={waveRef}>` block in `Deck.jsx` so each deck shows its own scrolling waveform with a centered playhead like before. User wanted the deck-internal layout back.
