...
- **2026-02-24 — Stacked dual-waveform beat-grid (P1).** Added `StackedWaveform.jsx` — a compact (~80px) strip at the top of the page showing Deck A (blue) over Deck B (red) with a centered white playhead and triangle markers. Each wavesurfer owns its own muted internal audio (sharing the deck's `<audio>` element caused Deck B to silently fail to render peaks). Scroll position is synced to each deck's `currentTime` via a single rAF loop. Deck.jsx dispatches `dj:track-loaded` with the playback URL; StackedWaveform listens and calls `ws.load(url)` on the right wavesurfer.
- **2026-02-24 — Vertical fader look/feel matched to crossfader.** Volume & tempo faders (`.fader-vert`) restyled to share the crossfader's chunky dark thumb (`36×28`, dark gradient, silver border, red glow) and flat `#1a1a1a` track. Switched from `appearance: auto` → `appearance: none` so the custom `::-webkit-slider-thumb` is honored in Chrome 124+, still using `writing-mode: vertical-lr; direction: rtl` for vertical orientation. Every fader now has identical tactile feel.
