...
- **2026-02-24 — Single-waveform UI (per-deck waves removed).** With the stacked beat-grid carrying both tracks, the per-deck wavesurfers were redundant and visually noisy. Removed the `<div ref={waveRef}>` block + `WaveSurfer.create` call from `Deck.jsx`. Stacked rows now `interact: true` and propagate clicks to each deck via a new `dj:stacked-seek` window event so scrubbing still works. Stacked rows bumped to 40px each for readability.
- **2026-02-24 — Stacked dual-waveform beat-grid (P1).** Added `StackedWaveform.jsx` — a compact strip at the top of the page showing Deck A (blue) over Deck B (red) with a centered white playhead and triangle markers. Each wavesurfer owns its own muted internal audio (sharing the deck's `<audio>` element caused Deck B to silently fail to render peaks). Scroll position is mirrored from each deck's `currentTime` + `playing` state via a single rAF loop so `autoScroll` engages correctly.
- **2026-02-24 — Vertical fader look/feel matched to crossfader.** Volume & tempo faders (`.fader-vert`) restyled to share the crossfader's chunky dark thumb, flat `#1a1a1a` track, red glow.
