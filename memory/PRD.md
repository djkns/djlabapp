...
- **2026-02-24 — Auto BPM detection (P1).** Added `web-audio-beat-detector@8.2.36` (pure-JS, no service). When a track loads on a deck, `Deck.jsx` fetches the audio bytes once, decodes them via the shared `AudioContext`, and runs `analyzeBPM(audioBuffer)` in a fire-and-forget background task. If the result lands within 60–200 BPM and the user hasn't swapped tracks, `baseBPM` is updated automatically (rounded to 1 decimal). A small orange `Activity` icon next to the BPM display indicates analysis in progress (`data-testid="deck-{a|b}-bpm-analyzing"`). Verified in browser: Deck A "01 - brown sugar" → 166.1, Deck B "06 - mary go round" → 141.5.
- **2026-02-24 — Reverted stacked beat-grid (user preference).** Removed `StackedWaveform.jsx` and the `<StackedWaveform />` mount. Restored per-deck `WaveSurfer` + the `<div ref={waveRef}>` block in `Deck.jsx` so each deck shows its own scrolling waveform with a centered playhead like before.
